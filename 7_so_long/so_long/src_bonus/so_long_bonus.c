...
/*   Updated: 2021/12/04 13:13:52 by jcarrete         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "so_long_bonus.h"

static void	set_font(t_so_long_game *game)
{
	if (LINUX == TRUE)
		mlx_set_font(game->mlx_data.mlx, game->mlx_data.win, FONT);
}
...
static void	initialize_window(t_so_long_game *game)
{
	...
	game->mlx_data.win = mlx_new_window(game->mlx_data.mlx, \
		game->map_data.width * TILE_SIZE, \
		game->map_data.height * TILE_SIZE, WINDOW_TITLE);
	if (game->mlx_data.win == NULL)
		exit_program(game, F_WIN, E_WIN, strerror(errno));
	set_font(game);
	game->map_data.monsters_init = TRUE;
